...
import { adminMedicalRecordQueries } from "@/api/admin/records/queries";
import type { MedicalRecord } from "@/api/admin/records/types";
...
function AdminMedicalRecordsPage() {
	const recordsQuery = useQuery({
		...adminMedicalRecordQueries.list(),
		enabled: typeof window !== "undefined",
	});
	const [selectedRecordId, setSelectedRecordId] = useState<number | null>(null);
	const selectedRecordQuery = useQuery({
		...adminMedicalRecordQueries.detail(selectedRecordId ?? 0),
			...
			<section className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
				<div className="space-y-2">
					...
					<p className="max-w-2xl text-sm text-muted-foreground">
						Lihat seluruh rekam medis pasien yang sudah dicatat dokter.
					</p>
				</div>
			</section>

			{/* <section className="grid gap-3 rounded-lg border bg-muted/20 p-4 text-sm md:grid-cols-3">
				<FilterPlaceholder label="Pasien" />
				<FilterPlaceholder label="Dokter" />
				<FilterPlaceholder label="Rentang tanggal" />
			</section> */}
		...
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent className="sm:max-w-2xl">
				<DialogHeader>
					...
					<DialogDescription>
						Data pemeriksaan dari appointment yang sudah selesai.
					</DialogDescription>
				</DialogHeader>
...
function formatOptionalDate(value: string | null) {
	return value ? recordDateFormatter.format(new Date(value)) : "-";
}
